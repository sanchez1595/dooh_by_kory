  ...
  return (
    ...
    >
      <div
        ...
        className={`relative flex items-center justify-center ${compact ? "h-[92px]" : "h-[164px]"}`}
      >
        <span
          className={`absolute top-2 left-2 rounded-full bg-[rgba(13,13,13,0.55)] font-semibold tracking-[0.04em] text-white backdrop-blur-xs ${
            compact ? "px-2 py-[2px] text-[9.5px]" : "top-2.5 left-2.5 px-[9px] py-[3px] text-[10.5px]"
          }`}
        >
          {valla.tipo}
        </span>
        <button
          ...
          className={`absolute top-2 right-2 flex cursor-pointer items-center justify-center rounded-full bg-white/90 ${
        ...
        >
          <svg
            ...
            className={compact ? "h-[13px] w-[13px]" : "h-[15px] w-[15px]"}
            strokeWidth={2}
          >
        ...
        <span
          className={`absolute right-2 bottom-2 rounded-full bg-white/90 font-mono font-bold text-ink ${
            compact ? "px-2 py-[2px] text-[9.5px]" : "right-2.5 bottom-2.5 px-[9px] py-[3px] text-[10.5px]"
          }`}
        >
          {valla.dim}
        </span>
      ...
      {compact ? (
        <div className="flex flex-col gap-[3px] px-3 pt-2.5 pb-3">
          <div className="flex items-baseline justify-between gap-1.5">
            <span className="truncate text-[13px] font-bold text-ink">{valla.nombre}</span>
            <span className="shrink-0 text-[11px] font-semibold text-ink">★ {valla.rating}</span>
          </div>
          <span className="truncate text-[11px] text-slate-500">{valla.ubicacion}</span>
          <span className="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-600">
            <b className="font-mono font-semibold">{valla.imp}</b> imp/día
